...
### 设计原则（v1.0，2026-05-08 SWISS KRONO case 定稿）

**核心定位**：这不是内部分析文档，是面向付费用户的情报产品。用户付了钱，他想感受到"有人替我做了我做不到的事"。

**语气标准**：冷静、精准、有洞察的情报分析师。不是咨询顾问的热情推销。
- 从"描述"改"判断"：~~"他是 CFO"~~ → "他的收件箱几乎是空的，162 个人都没联系他"
- 从"建议"改"指令"：~~"建议联系他"~~ → "第二段直接引原文。他会知道你做了功课"
- 从"罗列"改"故事"：~~"公司成立于 1966 年"~~ → "做了 60 年木材生意，但从来没有过一个 CIO"

**视觉节奏**：
- 正文用窄栏（720px），可视化组件用全宽（1100px），窄→宽→窄交替制造节奏
- Section 间距大（80px），留白充分
- 配色只用黑白灰，唯一强调色用在 IT 团队节点（蓝色虚线）
- 关键数字用超大字体（56px+, font-weight: 200 极细体）制造数据仪表盘感

### 报告结构（单页 HTML，不拆分多个文件）

**Section 0 · 封面（Hero）**

```
Prepared for
[用户全名]                          ← 40px, font-weight: 600
[用户 headline / 一句话身份]          ← 14px, #666
────────────────                    ← 分割线
[岗位标题]                          ← 28px
[公司 | 地点 | 日期]

[一句话核心判断]                     ← 20px, 不是一段话，是一句让人"哦"的话

┌─────────┬─────────┬─────────┐
│  大数字   │  大数字   │  大数字   │    ← 56px, weight 200
│  小标签   │  小标签   │  小标签   │    ← 13px, #666
└─────────┴─────────┴─────────┘
```

- 个人 case（有简历）：写用户名，第二人称
- Generic case（无简历）：不写名字，第三人称 + 故事化
- 三个大数字必须是反直觉的、让人"哦"的数据点（不是普通统计）
- slug: 个人 = `jobanalysis/{Org}{User}/`，通用 = `jobanalysis/{Org}/`

**Section 1 · "你不知道的 N 件事"（Insight Cards）**
...
│      副标题（一句话解释为什么反直觉）
│      ────────────────────────────────────
│      [展开后的分析段落，告诉用户"这意味着什么 + 你怎么用"]
└──────────────────────────────────────────────
```

设计要求：
- 标题必须是结论，不是描述。"60年，0个CIO" 而不是 "公司历史"
- 默认折叠，点击展开（CSS `max-height` transition）
- 展开内容必须回答"so what"——这对我意味着什么、我该怎么利用这个信息

**Section 2 · 决策人地图（组织架构图，SVG）**

- 竖向树形布局，SVG `viewBox` 内联
- 只画和这个岗位直接相关的人（5-8 个节点）
- 决策人节点用加粗边框（stroke-width: 2.5）
- 用户目标岗位用虚线框 + "你的目标岗位"标签
- 实线 = 直接汇报，虚线 = 战略方向/虚线汇报
- 下方加一个 highlight box 解释"为什么是 X 而不是 Y"
- **不用 ASCII，不用 D3（组织架构是静态的，SVG 更轻更可控）**

**Section 3 · 隐藏关系网络（D3.js 力导向图）**

与组织架构图的区别：
- 组织架构图 = 官方地图（汇报关系、预算审批链）
- 关系网络 = 暗线地图（跨实体协作、帖子发现的关联、IT ownership）

Section 标题用"官方架构之外"或"你在架构图上看不到的关系"，强化与上一节的差异。

节点规范：
- 黑色大圆 = 优先联系目标
- 灰色半透明 = 相关管理层（不是"组织上下文"——这个词读者看不懂）
- 蓝色 = IT 团队（潜在同事）
- 蓝色虚线 = 跨实体 IT ownership
- Hover 弹出详情 tooltip

**Section 4 · "你的 N 条路径"（联系目标）**

不叫"优先联系目标"，叫"你的 N 条路径"——每条路径是一个完整闭环。

分类规则：
- **直接联系目标**（hiring manager / 可直接 connect 的人）→ target-card，加粗边框
- **Plan B**（HR / 正式渠道）→ 普通 target-card
- **暗线情报**（层级太高不适合直接联系，但他的偏好/价值观影响招聘决策）→ dark-box，标题写"暗线情报 — XX 的偏好画像"，内容结构：他是谁 → 他的公开价值观（引原话）→ "怎么用"（指导你跟 hiring manager 说话的方式）

**不要把所有发现的人都列为"联系目标"。** 层级太高的人（CEO/集团 CFO/20K+ followers）不适合直接联系，但他们的公开言论和价值观是联系信写作和面试准备的暗线指导。

**Section 5 · "你的作战计划"（匹配度 + 时间线）**

匹配度不用表格，用竖排卡片：
- 强匹配（前 N 个）→ 黑底白字卡片，视觉上"实"
- 中等 → 白底灰边
- 弱匹配 → 浅灰底，文字也浅
- 弱项后面跟一个 highlight box 解释"为什么这不是致命伤"

行动时间线用简洁的竖向 timeline：`第1天 | 第2天 | 第5天 | ...`

最后一个 dark-box 做战略备注——告诉用户为什么这个方案 work 的底层逻辑。

### 样式

Read `~/.claude/style-templates/vercel.css` 作为 CSS 基础，在此之上自定义：
- `.hero` 封面区
- `.insight-card` 可展开卡片（onclick toggle `.open` class）
- `.org-svg-container` SVG 组织图容器
- `.graph-container` D3 关系图容器
- `.target-card` / `.target-card.primary` 联系目标卡片
- `.dark-box` 暗线情报 / 战略备注
- `.stats-row` / `.stat-card` / `.stat-number` 大数字统计

### 落盘

| 文件 | 内容 |
|---|---|
| `deliverable/index.html` | 完整报告（单页，包含组织架构 + 关系图谱） |
